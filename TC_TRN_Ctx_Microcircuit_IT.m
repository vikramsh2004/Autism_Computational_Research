%% Corticothalamic-Thalamocortical 3-Neuron Microcircuit with T-type Ca2+ Channels
% The circuit consists of three interconnected neurons:
%   1. Cortical (Ctx) Neuron
%   2. Thalamic Cortical (TC) Relay Neuron  (+ I_T, I_H)
%   3. Thalamic Reticular Nucleus (TRN) Neuron (+ I_T, I_H)
%
...
% T-type calcium channels (TC and TRN)
gT_TC = 1.0;   gT_TRN = 1.2;   ECa = 120; % mS/cm^2, mV

% H-current (supports hyperpolarization and rebound with I_T)
gH_TC = 0.05;  gH_TRN = 0.04;  EH = -43; % mS/cm^2, mV
...
% T-type h-gates and H-current r-gates (TC and TRN)
ht_TC = zeros(1, nt); ht_TRN = zeros(1, nt);
r_TC = zeros(1, nt);  r_TRN = zeros(1, nt);
...
ht_TRN(1) = hInf_T(v_TRN(1));
r_TC(1) = rInf_H(v_TC(1));
r_TRN(1) = rInf_H(v_TRN(1));
...
I_T_TC = zeros(1, nt); I_T_TRN = zeros(1, nt);
I_H_TC = zeros(1, nt); I_H_TRN = zeros(1, nt);
...
for k = 1:nt-1
    ...
    %% T-type and H-current gating updates (TC and TRN)
    ht_TC(k+1) = ht_TC(k) + dt * ((hInf_T(v_TC(k)) - ht_TC(k)) / tauH_T(v_TC(k)));
    ht_TRN(k+1) = ht_TRN(k) + dt * ((hInf_T(v_TRN(k)) - ht_TRN(k)) / tauH_T(v_TRN(k)));

    r_TC(k+1) = rInf_H(v_TC(k)) + (r_TC(k) - rInf_H(v_TC(k))) * exp(-dt / tauR_H(v_TC(k)));
    r_TRN(k+1) = rInf_H(v_TRN(k)) + (r_TRN(k) - rInf_H(v_TRN(k))) * exp(-dt / tauR_H(v_TRN(k)));
    ...
    % T-type calcium and H-current (TC and TRN)
    I_T_TC(k)  = gT_TC  * mInf_T(v_TC(k))^2  * ht_TC(k)  * (ECa - v_TC(k));
    I_T_TRN(k) = gT_TRN * mInf_T(v_TRN(k))^2 * ht_TRN(k) * (ECa - v_TRN(k));
    I_H_TC(k)  = gH_TC  * r_TC(k)  * (EH - v_TC(k));
    I_H_TRN(k) = gH_TRN * r_TRN(k) * (EH - v_TRN(k));
    ...
    v_TRN(k+1) = v_TRN(k) + dt * ((I_ext_TRN(k) + I_syn_TRN(k) + I_T_TRN(k) + I_H_TRN(k) - INa_TRN - IK_TRN - IL_TRN) / Cm);
end
...
I_T_TRN(end) = gT_TRN * mInf_T(v_TRN(end))^2 * ht_TRN(end) * (ECa - v_TRN(end));
I_H_TC(end)  = gH_TC  * r_TC(end)  * (EH - v_TC(end));
I_H_TRN(end) = gH_TRN * r_TRN(end) * (EH - v_TRN(end));
...
fprintf('--- T-type Ca2+ Peak Currents ---\nTC I_T peak: %.2f | TRN I_T peak: %.2f uA/cm^2\n', max(I_T_TC), max(I_T_TRN));
...
%% T-type Calcium and H-current Plot
figure('Name', 'T-type Ca2+ and H-currents', 'Color', 'w', 'Position', [120 120 800 700]);

subplot(4, 1, 1)
plot(time, I_T_TC, 'r', 'LineWidth', 1.2, 'DisplayName', 'TC I_T'); hold on
plot(time, I_T_TRN, 'b', 'LineWidth', 1.2, 'DisplayName', 'TRN I_T')
title('T-type Calcium Currents (I_T)')
xlabel('Time (ms)'); ylabel('I_T (\muA/cm^2)'); legend('Location', 'best'); grid on

subplot(4, 1, 2)
plot(time, ht_TC, 'r', 'LineWidth', 1.2, 'DisplayName', 'TC h_T'); hold on
plot(time, ht_TRN, 'b', 'LineWidth', 1.2, 'DisplayName', 'TRN h_T')
title('T-type Inactivation Gate (h_T)')
xlabel('Time (ms)'); ylabel('h_T'); legend('Location', 'best'); grid on

subplot(4, 1, 3)
plot(time, I_H_TC, 'r', 'LineWidth', 1.2, 'DisplayName', 'TC I_H'); hold on
plot(time, I_H_TRN, 'b', 'LineWidth', 1.2, 'DisplayName', 'TRN I_H')
title('H-current (I_H)')
xlabel('Time (ms)'); ylabel('I_H (\muA/cm^2)'); legend('Location', 'best'); grid on

subplot(4, 1, 4)
plot(time, mInf_T(v_TC), 'r--', 'LineWidth', 1.0, 'DisplayName', 'TC m_{\infty,T}'); hold on
plot(time, mInf_T(v_TRN), 'b--', 'LineWidth', 1.0, 'DisplayName', 'TRN m_{\infty,T}')
title('T-type Activation Steady State (m_{\infty,T}, instantaneous)')
xlabel('Time (ms)'); ylabel('m_{\infty,T}'); legend('Location', 'best'); grid on
