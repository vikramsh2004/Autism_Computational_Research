...
rebound_window = time >= 180 & time < 230;
...
fprintf('TC rebound spikes (180-230 ms): %d\n', sum(TC_spike_events(rebound_window)));
fprintf('TC min V during TRN inhibition: %.2f mV\n', min(v_TC(time >= 150 & time < 180)));
fprintf('TC max h_T during TRN inhibition: %.3f\n', max(hT_TC(time >= 150 & time < 180)));
