...
% Adds a low-threshold T-type calcium current to the TC relay neuron.
clear; clc
...
% Low-threshold T-type Ca2+ current in TC.
% IT_TC is written in the outward-current convention: gT*mT^2*hT*(V-ECa).
% During TRN inhibition hT deinactivates; after release, mT activates and
% the negative calcium current depolarizes TC enough to create a rebound burst.
gT_TC = 3.0;    % mS/cm^2
ECa_TC = 120;   % mV
...
% Prolonged TRN drive produces TC hyperpolarization through GABA release.
trn_drive_start = 120; % ms
trn_drive_end = 220;   % ms
I_ext_TRN(time >= trn_drive_start & time < trn_drive_end) = 14;

% No direct TC depolarizing pulse: rebound is generated by release from
% inhibition plus TC T-type calcium channel deinactivation.

% Protocol toggle (disabled for this rebound experiment).
...
Cmax_GABAa = 1; Cdur_GABAa = 1; alpha_GABAa = 5; beta_GABAa = 0.4; E_GABAa = -84;
...
g_GABAa_TRN_TC = 2.00; g_GABAb_TRN_TC = 0.04;
...
mT_TC = zeros(1, nt); hT_TC = zeros(1, nt);
...
inhibition_window = time >= trn_drive_start & time < trn_drive_end;
rebound_window = time >= trn_drive_end & time < trn_drive_end + 60;
...
fprintf('TC rebound spikes (%.0f-%.0f ms): %d\n', trn_drive_end, trn_drive_end + 60, sum(TC_spike_events(rebound_window)));
fprintf('TC min V during TRN inhibition: %.2f mV\n', min(v_TC(inhibition_window)));
fprintf('TC max h_T during TRN inhibition: %.3f\n', max(hT_TC(inhibition_window)));
fprintf('Peak inward TC I_T after release: %.2f uA/cm^2\n', min(I_T_TC(rebound_window)));

%% Plotting results
figure('Name', '3-Neuron Corticothalamic Microcircuit with TC T-Type Ca2+', 'Color', 'w', 'Position', [100 100 900 760]);
subplot(4, 1, 1)
...
subplot(4, 1, 3)
yyaxis left
plot(time, I_T_TC, 'm', 'LineWidth', 1.2, 'DisplayName', 'TC I_T')
ylabel('I_T (\muA/cm^2)')
yyaxis right
plot(time, hT_TC, 'Color', [0.25 0.25 0.25], 'LineWidth', 1.2, 'DisplayName', 'TC h_T')
hold on
plot(time, mT_TC, '--', 'Color', [0.5 0.5 0.5], 'LineWidth', 1.0, 'DisplayName', 'TC m_T')
ylabel('T-type gates')
title('TC T-Type Calcium Current and Gates')
...
function [mT0, hT0] = init_ttype_gates(V)
    [mT_inf, hT_inf] = ttype_rates(V);
    mT0 = mT_inf; hT0 = hT_inf;
end
...
function [mT_inf, hT_inf, tau_mT, tau_hT] = ttype_rates(V)
    % Low-threshold thalamic T-current kinetics adapted from common TC-cell
    % reduced models (Destexhe/Huguenard style), with V in mV and tau in ms.
    mT_inf = 1 / (1 + exp(-(V + 59) / 6.2));
    hT_inf = 1 / (1 + exp((V + 83) / 4));
    tau_mT = 0.22 + 0.15 / (exp((V + 27) / 10) + exp(-(V + 102) / 15));
    tau_hT = 30 + 90 / (1 + exp((V + 80) / 4));
    tau_mT = max(tau_mT, 1e-3);
    tau_hT = max(tau_hT, 1e-3);
end
